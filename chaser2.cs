...
public class chaser2 : MonoBehaviour {
    ...
    void Update()
    {
        if (players)
        {
            ...
            else if (Vector3.Distance(goal1.position, gameObject.transform.position) < Vector3.Distance(goal2.position, gameObject.transform.position))
            {
                agent.SetDestination(goal1.position);
                animator.SetFloat("Speed", 1);
                animator.SetBool("attack", false);
            }
            else if (Vector3.Distance(goal2.position, gameObject.transform.position) < Vector3.Distance(goal1.position, gameObject.transform.position))
            {
                agent.SetDestination(goal2.position);
                animator.SetFloat("Speed", 1);
                animator.SetBool("attack", false);
